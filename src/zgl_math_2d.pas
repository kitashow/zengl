...
type
  ...
  {$Else}
  zglTPoints2D = array[ 0..65535 ] of zglTPoint2D;//не знаю сколько выделять
  {http://zengl.org/forum/index.php/topic,297.msg4060.html#msg4060
  Проблема с ним из-за того, что я решил обойти ограничения Pascal'я,
  но не учёл момент с выходом за пределы массива(по хорошему неправ я,
  а не отладчик, который не в курсе, что его обманывают указателями :)).
  Можно попробовать решить проблему вместо 'R-' задав большую границу
  для типа zglTPoints2D(там 0..0, можно поставить что-то вроде 65536 или больше),
  но ни в коем случаи не использовать тип напрямую(компилятор будет выделять памяти дофига),
  а только zglPPoints2D с выделением памяти через zgl_GetMem и очисткой zgl_FreeMem.
  Проверки границы для него не будет, но для нормальных массивов в коде она останется.}
  {$EndIf}
